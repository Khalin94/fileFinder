...
<form name="formName" method="<%=nads.lib.reqsubmit.EnvConstants.FORM_METHOD%>" action="<%=request.getRequestURI()%>"><!--요구함 신규정보 전달 -->
      ...
      <div id="contents">
		...
		%>
        <!-- 리스트 버튼-->
			 <span class="list_bt"><a href="javascript:gotoMemEditPage()">요구함 수정</a></span>
			 <span class="list_bt"><a href="javascript:gotoMemDeletePage(<%=objRs.getTotalRecordCount()%>)">요구함 삭제</a></span>
		<%
				} // 등록자와  로그인자가 같을때만 화면에 출력함.
		%>
			 <span class="list_bt"><a href="javascript:copyReqBox('<%=objRsSH.getObject("REQ_BOX_ID")%>')">요구함 복사</a></span>
			 <span class="list_bt"><a href="javascript:gotoList()">요구함 목록</a></span>
		<%
		...
		%>
			...
			<% } else { %>
		     ...
		     <span class="list_bt"><a href="javascript:sendReqDocNew('<%= CodeConstants.REQ_DOC_TYPE_001 %>', '<%= (String)objParams.getParamValue("ReqBoxID") %>')">요구함 발송</a></span>
				 <%}else{%>
			 <span class="list_bt"><a href="javascript:sendReqDocNew('<%= CodeConstants.REQ_DOC_TYPE_001 %>', '<%= (String)objParams.getParamValue("ReqBoxID") %>')">의원실 명의 요구서 발송</a></span>
			<%}%>
		<% } %>
		<%
					if(objCmtSubmt.checkCmtOrganMakeAutoSche((String)objRsSH.getObject("CMT_ORGAN_ID")) && objUserInfo.getIsMyCmtOrganID((String)objRsSH.getObject("CMT_ORGAN_ID"))) {
		%>
			 <span class="list_bt"><a href="javascript:sendReqDocByCmtTitle()">위원회 명의 요구서 발송</a></span>
		<%
			}
		%>
		<%
		...
		%>
			 <span class="list_bt"><a href="javascript:PreReqDocView(formName,'<%= (String)objParams.getParamValue("ReqBoxID") %>', '<%= strReqTp %>')">요구서 미리 보기</a></span>
		<%
