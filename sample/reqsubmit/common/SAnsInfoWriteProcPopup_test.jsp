...
<%
	...
	try {
		...
		System.out.println("kangthis logs 222=> "+multi.getParameter("_innods_orifilename")); //정상나옴.hwp
		System.out.println("kangthis logs 333=> "+multi.getParameter("_innods_oripdffilename")); //정상나옴.pdf
		...
		String[] strFilesize = multi.getParameterValues("_innods_filesize");
                String[] srtOriName = multi.getParameter("_innods_orifilename").split("Ω");
        	String[] srtPdfName = multi.getParameter("_innods_oripdffilename").split("Ω");
		...
		for(int i=0;i<strFilename.length;i++){
			...
			//strOrgFileName = srtOriName[i];
			//strOrgPdfFileName = srtPdfName[i];
//System.out.println("strOldPdfFile:"+strOldPdfFile);
//System.out.println("strOldPdfFile:"+(FileUtil.getFileExtension(strOldPdfFile)));
			...
			if(i != 0 && i != 1){
System.out.println("kangthis logs 111 i => " + i);
				if(i%2 ==0){
...
System.out.println("kangthis logs 111 strPdfFileName => " + strPdfFileName);
						System.out.println("kangthis logs 111 PdfFilePath => " + PdfFilePath);
					}else{
...
System.out.println("kangthis logs 222 strDocFileName => " + strDocFileName);
						System.out.println("kangthis logs 222 DocFilePath => " + DocFilePath);
					}
					
				}else{
					strDocFileName = strDocFileName+"Ω"+FileUtil.getFileName(strOldDocFile);
					DocFilePath = DocFilePath+"Ω"+ strFolder + "/" + FileUtil.getFileName(strOldDocFile, "/");  //"Ω"+
System.out.println("kangthis logs 333 strDocFileName => " + strDocFileName);
					System.out.println("kangthis logs 333 DocFilePath => " + DocFilePath);
				}
			}else if(i==0 || i==1){
System.out.println("kangthis logs 222 i => " + i);
				if((FileUtil.getFileExtension(strOldPdfFile)).equals("pdf")||(FileUtil.getFileExtension(strOldPdfFile)).equals("PDF")){
					if(strOldPdfFile.toLowerCase().indexOf("ori_") == -1){
						strPdfFileName = FileUtil.getFileName(strOldPdfFile);
						PdfFilePath = strFolder + "/" + FileUtil.getFileName(strOldPdfFile, "/");
System.out.println("kangthis logs 444 strPdfFileName => " + strPdfFileName);
						System.out.println("kangthis logs 444 PdfFilePath => " + PdfFilePath);
					}else{
						strDocFileName = FileUtil.getFileName(strOldDocFile);
						DocFilePath = strFolder + "/" + FileUtil.getFileName(strOldDocFile, "/");
System.out.println("kangthis logs 555 strDocFileName => " + strDocFileName);
						System.out.println("kangthis logs 555 DocFilePath => " + DocFilePath);
					}
					
				}else{
					strDocFileName = FileUtil.getFileName(strOldDocFile);
					DocFilePath = strFolder + "/" + FileUtil.getFileName(strOldDocFile, "/");
System.out.println("kangthis logs 666 strDocFileName => " + strDocFileName);
					System.out.println("kangthis logs 666 DocFilePath => " + DocFilePath);
				}
		...
		System.out.println("strDocFileName :"+strDocFileName);
                //System.out.println("strOrgFileName :"+strOrgFileName);
		//System.out.println("strOrgPdfFileName :"+strOrgPdfFileName);
...
<script language="javascript">
	...
	function autoSubmit(form) {
		...
		alert('kangthis logs ' + f2.OrgRealFileName.value);
		alert('kangthis logs ' + f2.PdfRealFileName.value);
		f2.MsgDigest.value = "undefinedΩ";
		f2.user_dn.value = form.user_dn.value;
		f2.count.value = "<%=strFilename.length/2%>";
		f2.action = "/reqsubmit/common/SAnsInfoWriteProc_test.jsp";
		f2.submit();
		//self.close();
	}
</script>
