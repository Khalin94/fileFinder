...
<%
	...
	try {
		...
		if(objUserInfo.getCurrentCMTList().isEmpty()){
			...
			objCmtRs = new ResultSetHelper(objReqBox.getReqrPerYearCMTList(lst,CodeConstants.REQ_ING_STT_002, CodeConstants.REQ_BOX_STT_006));   
		} else {  	
		...
		if(strSelectedAuditYear.equals("")){
			String strYear = (StringUtil.getSysDate()).substring(0,4);
			objParams.setParamValueIfNull("AuditYear",strYear);
		}
...
<html>
<head>
<title><%=MenuConstants.REQUEST_BOX_COMM%> > <%=MenuConstants.REQ_BOX_SEND_END%></title>
...
<form name="listqry" method="post" action="<%=request.getRequestURI()%>">
        ...
        <tr> 
            ...
            <tr> 
            	<td width="30%" background="/image/reqsubmit/bg_reqsubmit_tit.gif" height="23">
                	<!-------------------- 타이틀을 입력해 주세요 ------------------------>
                	<span class="title"><%=MenuConstants.REQ_BOX_SEND_END%></span> <strong>- 요구함 목록</strong>
                </td>
				...
				<!-------------------- 현재 위치 정보를 기술한답니다. ------------------------>
                    ...
                    <%=MenuConstants.GOTO_HOME%> > <%=MenuConstants.REQ_SUBMIT_MAIN_MENU%> > <%=MenuConstants.REQUEST_BOX_COMM%> > <B><%=MenuConstants.REQ_BOX_SEND_END%></B>
                </td>
			</tr>
			</table></td>
		</tr>
		<tr> 
            ...
            <!-------------------- 현재 페이지에 대한 설명 기술 ------------------------>
            제출기관에서 발송한 요구함 목록을 확인하실 수 있습니다.
			</td>
		</tr>
		<!---------------------------------------------------------------------- 재정의된 조회 조건 라인 시작 ------------------------------------------------------------------------>
              <tr> 
		...
		<tr> 
            ...
            <tr align="center" class="td_top"> 
            	<td width="15" align="center"><input type="checkbox" name="checkAll" onClick="javascript:checkAllOrNot(document.listqry)"></td>
	          	<td width="34" align="center" height="22">NO</td>
        	  	<td width="320" height="22"><%=SortingUtil.getSortLink("changeSortQuery","REQ_BOX_NM",strCommReqBoxSortField,strCommReqBoxSortMtd,"요구함명")%></td>
              	<td width="175" height="22"><%=SortingUtil.getSortLink("changeSortQuery","SUBMT_ORGAN_NM",strCommReqBoxSortField,strCommReqBoxSortMtd,"제출기관")%></td>
              	<td width="75" height="22"><%=SortingUtil.getSortLink("changeSortQuery","RLTD_DUTY",strCommReqBoxSortField,strCommReqBoxSortMtd,"업무구분")%></td>				
             	<td width="70" align="center" height="22">제출/요구</td>
              	<td width="70" align="center" height="22"><%=SortingUtil.getSortLink("changeSortQuery","LAST_REQ_DOC_SND_DT",strCommReqBoxSortField,strCommReqBoxSortMtd,"발송일시")%></td>
	        </tr>
    	    <tr> 
            ...
            <tr onMouseOver="this.style.backgroundColor='#FCFDF0'" onMouseOut="this.style.backgroundColor=''"> 
            	<td width="15" align="center"><input type="checkbox" name="ReqBoxID" value="<%=strReqBoxID%>"></td>
              	<td width="34" align="center" height="20" align="center"><%= intRecordNumber %></td>
 				...
 				<td width="75" class="td_lmagin"><%=objCdinfo.getRelatedDuty((String)objRs.getObject("RLTD_DUTY"))%></td>
          		<td width="70" class="td_lmagin" align="center"><%=objRs.getObject("SUBMT_CNT")%> / <%=objRs.getObject("REQ_CNT")%></td>
