...
<%
	...
	try {
		...
		reqInfoID = multi.getParameter("ReqInfoID");
		...
		if ("".equalsIgnoreCase(StringUtil.getEmptyIfNull(reqInfoID))) {
	 	...
	 	if (CodeConstants.ANS_MTD_ELEC.equalsIgnoreCase(ansType)) { // '전자 파일'의 경우
	 		...
	 		msgDigest = selfDelegate.getMsgDigest(savePath+pdfFile);
			...
			out.println("<input type=\"hidden\" name=\"ReqInfoID\" value=\""+reqInfoID+"\">\n");
...
<script language="javascript">
	...
	function autoSubmit(str) {
		...
		f.action = "SBasicAnsInfoRegistProc2.jsp";
		f.submit();		
...
<%
			...
			int dbSuccess = selfDelegate.insertAnswerInfo(param);
			
			if (dbSuccess > 0) {
				out.println("<script>alert('비전자 문서 답변 제출을 정상적으로 완료하였습니다.');</script>");
				out.println("<meta http-equiv='refresh' content='0; url=SMakeReqInfoVList.jsp?ReqInfoID="+reqInfoID+"&ReqBoxID="+reqBoxID+"'>");
			} else {
