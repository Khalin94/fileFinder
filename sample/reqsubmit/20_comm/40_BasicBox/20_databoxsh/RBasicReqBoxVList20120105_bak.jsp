...
<form name="formName" method="<%=nads.lib.reqsubmit.EnvConstants.FORM_METHOD%>" action="<%=request.getRequestURI()%>"><!--요구함 신규정보 전달 -->
      ...
      <div id="contents">
		...
		%>
			...
			%>
		<!-- 요구함 수정 -->
			 <span class="list_bt"><a href="#" onclick="javascript:gotoEdit()">요구함 수정</a></span>
			 <span class="list_bt"><a href="#" onclick="javascript:gotoDel()">요구함 삭제</a></span>
			<%
					}
				}
			%>
			<!-- 요구함 복사 -->
			 <span class="list_bt"><a href="javascript:copyPreReqBox(formName)">요구함 복사</a></span>
			<%
			 }//end if
			} //end if 권한없음에 관련
			%>
