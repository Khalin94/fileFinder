...
<%
	...
	System.out.println("kangthis logs AnsDocSendProc.jsp AnswPermCD => " + strAnswPermCD);
...
<%
			...
			if (intResult < 1) {
				...
				strUserKind = StringUtil.getEmptyIfNull((String)hashSndrInfo.get("USER_KIND"));
				
				System.out.println("kangthis log SMS 답변=> strSendPhoneNo : " + strSendPhoneNo);
				System.out.println("kangthis log SMS 답변=> strReturnPhoneNo : " + strReturnPhoneNo);
				System.out.println("kangthis log SMS 답변=> strSendPhoneNo1 : " + strSendPhoneNo1);
				System.out.println("kangthis log SMS 답변=> strReturnPhoneNo1 : " + strReturnPhoneNo1);
				System.out.println("kangthis log SMS 답변=> strUserKind : " + strUserKind); 
...
//					System.out.println("[메일발송] 실제로는 여기서 에러가 발생했슴.... 확인바람  ");
				...
				if (CodeConstants.NTC_MTD_BOTH.equalsIgnoreCase(strSmsUse) && StringUtil.isAssigned(strSendPhoneNo)) {
					...
					if (strAnswPermCD.equals("222")) {
							String strSmsMsg = "의정자료전자유통시스템에 추가답변서가 제출되었습니다["+strSubmtOrganNm2+"]";
							hashSmsInfo.put("MSG", strSmsMsg);
						} else {
							String strSmsMsg = "의정자료전자유통시스템에 답변서가 제출되었습니다["+strSubmtOrganNm2+"]";
							System.out.println("답변메시지 : "+strReqBoxID +"-"+ strSmsMsg);
					...
					System.out.println("kangthis log SMS 답변=> hashSmsInfo.put : " + hashSmsInfo.put("USER_KIND", strUserKind));
					//hashSmsInfo.put("OFFI_DOC_ID", strOffiDocID);
...
//						System.out.println("[문자전송] 실제로는 여기서 에러가 발생했슴.... 확인바람  ");
				...
				out.println("alert('답변서 발송을 완료하였습니다. 발송하신 답변자료는 [제출완료요구함]에서 확인 가능합니다.');");
