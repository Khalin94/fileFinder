...
<%
	...
	strMacAddr = StringUtil.ReplaceString(strMacAddr, "\t", "");
	strMacAddr = StringUtil.ReplaceString(strMacAddr, " ", "");
	if (strMacAddr.length() > 12) strMacAddr = strMacAddr.trim().substring(0, 12);
	
	StringBuffer strSQL = new StringBuffer();
	DBAccess objDAO=null;

	//System.out.println("[ezPDFAnyWhere(open.jsp)] doc ID : "+strDocID);
	//System.out.println("[ezPDFAnyWhere(open.jsp)] Reg Key : "+strRegKey);
	//System.out.println("[ezPDFAnyWhere(open.jsp)] Mac Addr : "+strMacAddr);
	...
	try {
		...
		//System.out.println("[ezPDFAnyWhere(open.jsp)] execute SQL : " +strSQL.toString());
		...
		//System.out.println("[ezPDFAnyWhere(open.jsp)] Reg Key : " +strRsRegKey);
		...
		if (strRsRegKey.equalsIgnoreCase(strMacAddr)) { // 동일하다면 계속 진행
			...
			System.out.println("[ezPDFAnyWhere(open.jsp)] Hash Doc ID : " +intHashDocID);
			int intKeyNo = intHashDocID%1000; // 문자열의 hashcode에 대한 key를 구해올 자릿수를 구한다.
			//System.out.println("[ezPDFAnyWhere(open.jsp)] Key No : " +intKeyNo);
			
			strSQL.append("SELECT enc_key FROM TBDS_ezPDF_enc_key ");
			strSQL.append(" WHERE enc_key_id="+intKeyNo);
			
			//System.out.println("[ezPDFAnyWhere(open.jsp)] execute SQL2 : " +strSQL.toString());
			objDAO.setSQL(strSQL.toString());
			objDAO.doQuery();
			objHashResult = objDAO.getSingleHashtable();
			String strRsEncKey = (String)objHashResult.get("ENC_KEY");
			
			//System.out.println("[ezPDFAnyWhere(open.jsp)] Enc Key : " +strRsEncKey);
