...
<%@ page import="nads.dsdm.app.reqsubmit.delegate.requestinfo.prereqinfo.*" %>
...
<%
	...
	try {
		...
		SPreAnsInfoDelegate selfDelegate = new SPreAnsInfoDelegate();
		...
		reqInfoID = request.getParameter("ReqInfoID");
		...
		if ("".equalsIgnoreCase(StringUtil.getEmptyIfNull(reqInfoID))) {
			...
			throw new EJBExtendException("요구함 ID가 없습니다. 답변을 제출하실 수 없습니다.");
	 		...
	 		throw new EJBExtendException("Sign Data가 없습니다. 현재 단계에서는 없음 안돼부러~");
	 	}
 		...
 		if (HLSuccess < 1) {
 			throw new EJBExtendException("등록된 PDF 파일의 하일라이트 색인 과정에서 에러가 발생했습니다.");
 		}
	 		
 		// 작업.2. Full Text 색인을 위한 TXT 파일을 추출하자
 		idxFileName = selfDelegate.getIdxFile(pdfFile);
 		if ("".equalsIgnoreCase(idxFileName)) {
 			throw new EJBExtendException("TXT 파일 추출작업에서 에러가 발생했습니다.");
 		}
 		...
 		if (success1 && success2 && success3) { // 셋 다 성공해야 다음 프로세스를 진행하지...
			...
			paramII.add(""); 							// 9 - 발송 방법
 			...
 			if (dbSuccess1 > 0 && dbSuccess2 > 0) {
 				...
 				out.println("<meta http-equiv='refresh' content='0; url=SBasicReqInfoVList.jsp?ReqInfoID="+reqInfoID+"&ReqBoxID="+reqBoxID+"'>");
 			} else {
 				out.println("<script>alert('Sorry.............. Check Error..... and Please, try again.. ');</script>");
 			}
 			
	 	} else {
	 		throw new EJBExtendException("파일 복사 과정 중 에러 발생");
	 	}
