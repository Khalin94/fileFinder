	...
	<%
	 ...
	 %>
            <tr>
			  <%if(objUserInfo.getOrganGBNCode().equals("004")){%>
              <td >
			  <input type="checkbox" name="ReqInfoIDs" value="<%=strReqInfoID%>" class="borderNo"/></td>
			  <% }else{ %>
              <td></td>
			  <% }%>
			  <td><%= intRecordNumber %></td>
              <td><a href="JavaScript:gotoDetail('<%=strReqInfoID%>');"><%=StringUtil.substring((String)objRs.getObject("REQ_CONT"),30)%></a></td>
              <td><a href="javascript:gotoDetail_Box('<%=strReqBoxID%>')"><%=StringUtil.getReqBoxNmList((String)objRs.getObject("REQ_BOX_NM"))%></a>
			  </td>
		...
		if(!strReqSubmitFlag.equals("004")){
			 if(objUserInfo.getOrganGBNCode().equals("004")){
		%>
		<span class="right"> <span class="list_bt"><a href="javascript:addBinder(document.formName);">바인더담기</a></span> </span>
		<% }// getOrganGBNCode()
		}//권한 없음  %>
		</div>
