...
<%
	...
	objParams.setParamValue("ReqInfoSortField", "req_stt"); // 정렬순서. 최종답변일
...
<%
	...
	try{
		...
		objRsAuditYear = new ResultSetHelper(objReqInfo.getAuditYearDaeList(lst,strSubmtOrganId,strStartdate,strEnddate,CodeConstants.REQ_STT_NOT));
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="list01">
          <thead>
            <tr>
              ...
              <th scope="col"><%=SortingUtil.getSortLink("changeSortQuery","REG_DT",strReqInfoSortField,strReqInfoSortMtd,"요구일자")%></th>
            </tr>
          </thead>
          <tbody>
			<%
			...
			%>
			<tr>
					<td colspan="9" align="center">등록된 요구 목록이 없습니다.</td>
			</tr>
			<%
				} else {
				while(objRs.next()) {
            ...
            <tr>
              ...
              <td><%= StringUtil.getNotifyImg((String)objRs.getObject("REG_DT"), (String)objRs.getObject("REQ_STT")) %><a href="JavaScript:gotoDetail('<%= strReqInfoID %>', '<%= (String)objRs.getObject("REQ_BOX_ID")%>');" hint="<%= StringUtil.substring((String)objRs.getObject("REQ_DTL_CONT"), 80) %>"><%= (String)objRs.getObject("REQ_CONT") %></a></td>
              <td>
			  <%
              ...
              <td><a href="javascript:gotoReqBoxInfoView('<%= (String)objRs.getObject("REQ_BOX_STT") %>', '<%= (String)objRs.getObject("REQ_BOX_ID") %>')"><img src="/image/reqsubmit/icon_secretariat.gif" border="0" alt="<%=objRs.getObject("REQ_BOX_NM")%> 바로가기"></td>
