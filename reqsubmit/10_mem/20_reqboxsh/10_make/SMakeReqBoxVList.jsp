...
<script language="javascript">
	...
	function changeSortQuery(sortField, sortMethod){
  	...
  	function goSbmtReqForm() {
		...
		window.open("/blank.html","newpopup","resizable=0,menubar=0,status=0,titlebar=0,scrollbars=0,location=0,toolbar=0,height=600,width=595, left="+winl+", top="+winh);
		f.submit();
  	}
        // 답변 작성 화면으로 이동 <대용량 테스트틀 위함>
        function goSbmtReqForm1() {
                var cnt = 0;
                if(<%= intTotalRecordCount %> == 1) {
                        if(eval("document.viewForm.ReqInfoID.checked") == true) {
                                cnt = 1;
                ...
                f = document.viewForm;
                f.target = "newpopup";
                f.action = "/reqsubmit/common/SAnsInfoWrite_test.jsp";
                var winl = (screen.width - 600) / 2;
                var winh = (screen.height - 600) / 2;
                window.open("/blank.html","newpopup","resizable=no,menubar=no,status=no,titlebar=no,scrollbars=no,location=no,toolbar=no,height=580,width=595, left="+winl+", top="+winh);
                f.submit();
        }
  	// 구버전-답변 작성 화면으로 이동
			  ...
			  <td colspan="3">
				 <table width="100%" border="0" cellspacing="0" cellpadding="0" class="list_none">
					 ...
					 <tr>
						 ...
						 <td class="fonts"><%=(String)objRsSH.getObject("RCVR_NM")%> (<a href="mailto:<%=(String)objRsSH.getObject("RCVR_EMAIL")%>"><%=(String)objRsSH.getObject("RCVR_EMAIL")%></a> / <%=(String)objRsSH.getObject("RCVR_CPHONE")%>)</td>
					 </tr>
				 </table>
			</td>
...
%>
            <tr>
              <td><input name="ReqInfoID" type="checkbox" value="<%= strReqInfoID %>"  class="borderNo" onClick="javascript:checkBoxValidate(<%= (intRecordNumber-1) %>, '<%= strReqInfoID %>')" />
       ...
       <!-- 리스트 버튼-->
        ...
        <!-- /리스트 내 검색 -->
            ...
            <!-- 변경된 업로드 컴포넌트 에러시 goSbmtReqForm-> goSbmtReqForm_old 변경-->
                        <% if (objUserInfo.getUserID().equals("0000039930")) { %>
            ...
            <!-- <span class="list_bt"><a href="javascript:goSbmtReqForm1()">대용량파일등록_test</a></span> -->
            <% } else { %>
            <span class="list_bt"><a href="javascript:goSbmtReqForm_old()">답변작성</a></span>
			<span class="list_bt"><a href="javascript:viewReqHistory()">요구이력보기</a></span>
            <span class="list_bt"><a href="javascript:goSbmtReqForm()">대용량파일등록</a></span>            
