...
<form name="listForm" method="post" action="<%=request.getRequestURI()%>">
      ...
      <div id="pgTit" style="background:url(/images2/foundation/stl_bg02.gif) no-repeat left top;">
        ...
        <div class="navi"><img src="/images2/foundation/home.gif" width="13" height="11" /> <%=MenuConstants.GOTO_HOME%> > <%=MenuConstants.REQ_SUBMIT_MAIN_MENU%> > <%=MenuConstants.REQUEST_BOX_COMM%>
                        > <%=MenuConstants.REQUEST_BOX_PRE%> > <%=MenuConstants.REQ_INFO_LIST%> > <%=MenuConstants.REQ_INFO_LIST_SUBMT_DONE%></div>
        <p><!--문구--></p>
      </div>
      <!-- /pgTit -->
      ...
      <div id="contents">
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="list01">
          <thead>
            <tr>
				<th scope="col" style="width:15px;">
					<input name="checkAll" type="checkbox" value=""  class="borderNo" onClick="checkAllOrNot(document.listForm);" />
				</th>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td><a href="JavaScript:gotoDetail('<%= strReqInfoID %>');"><%= StringUtil.substring((String)objRs.getObject("REQ_CONT"), 30) %></a>
			  </td>
              <td><a href="javascript:gotoDetail_Box('<%=strReqBoxID%>');"><%=StringUtil.getReqBoxNmList((String)objRs.getObject("REQ_BOX_NM"))%></a></td>
		...
		%>
		<span class="right"><span class="list_bt"><a href="#" onclick="javascript:if(<%=intTotalRecordCount%>>0){addBinder(document.listForm);} else {alert('먼저 선택해 주세요')}">바인더 담기</a></span></span>
		<%
			}		//end if   권한없음에 관련
		%>
		</div>
