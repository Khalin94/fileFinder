...
%>
            <tr>
                <th height="25">&bull; 요구함명<%=FLAG%><%=objParams.getParamValue("CmtOrganID")%><%=resultCount%></th>
...
<!-- 중단 버튼 시작 -->
		...
		%>
			 <div class="mi_btn"><a href="#" onclick="GansaOk()"><span>간사 승인</span></a></div>
		<%}%>
		<%
		/** 위원회 행정직원 일때만 화면에 출력함.*/
		if(objUserInfo.getOrganGBNCode().equals("004") && !strReqSubmitFlag.equals("004")){
		...
		<!-- 요구서 발송 -->
			<div class="mi_btn"><a href="#" onclick="sendReqDoc('<%=CodeConstants.REQ_DOC_TYPE_002%>', '<%=objParams.getParamValue("ReqBoxID")%>')"><span>요구서 발송</span></a></div>
		<% } %>
		...
		%>
			<div class="mi_btn"><a href="#" onclick="ReqDocOpenView('<%=objRsSH.getObject("REQ_BOX_ID")%>','002')"><span>요구서 보기</span></a></div>
		<%} %>

		<%if(intTotalRecordCount > 0){%>
				<!-- 요구함 미리 보기 -->
			<div class="mi_btn"><a href="#" onclick="PreReqDocView(formName,'<%=objRsSH.getObject("REQ_BOX_ID")%>','002');"><span>요구함 미리 보기</span></a></div>
	 ...
	 <% }else{
		if(strOldReqOrganId.equals(strOrganID)){%>
			<div class="mi_btn"><a href="#" onclick="PreReqDocView(formName,'<%=objRsSH.getObject("REQ_BOX_ID")%>','002');"><span>요구서 미리보기</span></a></div>
	<%	}
	}%>
...
-->
        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="list01">
          <thead>
            <tr>
              <th scope="col"><a>구분</a></th>
              <th scope="col"><a>확인자정보</a></th>
              <th scope="col"><a>확인시간</a></th>
              <th scope="col"><a>기술사항</a></th>
            </tr>
          </thead>
          <tbody>
