...
<%
	...
	try{
		...
		objYearRs =  new ResultSetHelper(objReqInfo.getAuditYearDaeList(lst,strSubmtOrganId,strStartdate,strEnddate,CodeConstants.REQ_STT_SUBMT));
...
%>
      ...
      <div id="contents">
		...
		%>
            <select onChange="javascript:doRefresh()" name="CmtOrganID">
              <option selected="selected" value="">:::: 전체 위원회 ::::</option>
				<%
 ...
 <!------------------------- TAB에 해당하는 테이블(목록이든 등록폼이든 상세정보든) 출력 시~~~작 ------------------------->
          ...
          <thead>
            <tr>
			  <th scope="col" style="width:15px;">
				<input name="checkAll" type="checkbox" class="borderNo" onClick="checkAllOrNot(document.formName);" />
			  </th>
              ...
              <th scope="col"><a>답변</a></th>
               ...
               <th scope="col"><%=SortingUtil.getSortLink("changeSortQuery","LAST_REQ_DT",strCommReqInfoSortField,strCommReqInfoSortMtd,"요구일자")%></th>
               <th scope="col"><%=SortingUtil.getSortLink("changeSortQuery","LAST_ANS_DT",strCommReqInfoSortField,strCommReqInfoSortMtd,"답변일자")%></th>
            </tr>
          </thead>
          <tbody>
			<%
			 ...
			 %>
            <tr>
              <td><input name="ReqInfoIDs" type="checkbox" class="borderNo" value="<%=strReqInfoID%>" /></td>
              <td><%= intRecordNumber %></td>
              <td><a href="JavaScript:gotoDetail('<%=strReqBoxID%>','<%=strReqInfoID%>');"><%=StringUtil.substring((String)objRs.getObject("REQ_CONT"),30)%></a>
			  </td>
              <td><%=objRs.getObject("REQ_ORGAN_NM")%></td>
              <td><a href="JavaScript:gotoReqBoxInfoView('<%=strReqBoxStt%>','<%=strReqBoxID%>');"><img src="/image/reqsubmit/icon_secretariat.gif" border="0" alt="<%=objRs.getObject("REQ_BOX_NM")%> 바로가기"></a></td>             <td><%=nads.lib.reqsubmit.util.DBAccessUtil.makeAnsInfoHtml((String)objRs.getObject("ANS_ID"),(String)objRs.getObject("ANS_MTD"),(String)objRs.getObject("ANS_OPIN"),(String)objRs.getObject("SUBMT_FLAG"),objUserInfo.isRequester())%></td>
			  ...
			  <td><%=StringUtil.getDate2((String)objRs.getObject("LAST_ANS_DT"))%></td>
				...
				%>
			<tr>
				<td colspan="9" align="center">등록된 요구자료가 없습니다.</td>
			</tr>
				<%
					} // end if
				%>
        ...
        <!-- /리스트 내 검색 -->
		 ...
		 %>
		<span class="right"><span class="list_bt"><a href="#" onclick="javascript:addBinder(document.formName);">바인더 담기</a></span></span>
		<%
			 }
		 }
		 %>
