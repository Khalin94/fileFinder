...
<%
		    ...
		    //System.out.println("[RegFileOpen] db조회값 strReqCont = " + strReqCont);
		    intResult = PdfDelegate.updateQryCnt(strAnsId,strDoc);
...
<%
	...
	} else if( strDoc.compareTo("DOC") == 0 ){

		//System.out.println(" **원본문서 다운 시작 **");
		objHashtable =   PdfDelegate.getFileInfo(strAnsId); // 답변  id  = 1 로 셋팅
		int intCount =  objHashtable.size();
...
<%
            ...
            if (strDocFilePath.indexOf(".pdf")>0){
...
%>
                <script>
                    window.open('<%=strHttpFilePath%>','','toolbar=no,status=no,directories=no,width=750,height=820,scrollbars=yes,location=auto,resizable=no,menubar=no');
                    history.back();
                </script>
<%
            } else {
...
//	    	    strHttpFilePath = "/reqsubmit/common/FileDownLoad.jsp?DOC=" + strDocFilePath;
					...
					System.out.println("strDocFilePath============="+strDocFilePath);
					System.out.println("strDocFilePath============="+strDocFilePath.substring(strDocFilePath.lastIndexOf("/")+1));
					System.out.println("strReqCont============="+strReqCont);

					String saveFileName = strDocFilePath.substring(strDocFilePath.lastIndexOf("/")+1);
...
//					String fileName = strfilename;
					String fileName2 = new String(strDocFilePath.getBytes("8859_1"), "MS949");
