        ...
        <div id="rightCon">
            ...
            <div id="contents">
                ...
                <div class="serP">
							...
							<!-- 기존의 검색 셀렉트와 버튼만 넣으세요-->
							 <select name="deptText" class="select" onchange="search()">
							  ...
							  <% for(int i=0; i < objArrselectNews.size(); i++){
								...
								if(!((String)objHashBSorts.get("NAME")).equals("경향신문") && !((String)objHashBSorts.get("NAME")).equals("한겨레신문")){
							  %>
                    ...
                    <!-- /검색창  -->
					<%if(!queryText.equals("")){%>
					<span>검색어&nbsp;<span style="color:#ea5e11">"<%=replace(replace(toMulti(queryText), "<", "&lt;"),">","&gt")%>"</span>으로 <span style="color:#ea5e11">(<%=hitCount%>)</span>건수의 결과가 나왔습니다. </span>
                    <%}%>
					<ul class="result">
					<%
					if (hitCount !=0) {
