<!--
**      Program Name : ISearch_Relation.jsp
**      Program Date : 2004. 05.19.
**      Last    Date : 2004. 05.19.
**      Programmer   : Kim Kang Soo
**      Description  :상세검색중 관련기관웹사이트 검색
**
**      1)위원회 리스트를 끌어오는 부분에서 DB커넥션을 사용함
**      1)위원회별 개별 결과건수를 보여
-->
...
//문자파라미터
	...
	if(MENU_CD==null || MENU_CD.equals("")) MENU_CD = "03";
...
//숫자파라미터
	...
	ArrayList objArrselectCommitteeUnder;			

	try {
		
		objArrselectCommittee = objCMD.selectCommittee("A000000002");
...
<%	
	...
	try {
		...
		objArrselectCommitteeUnder = objCMD.selectCommitteeUnder(deptText);
...
<%	
		...
		//out.println("<br><font color=red>00: deptText= "+deptText+"</font><br>");	
		//out.println("<br><font color=red>01: deptText00= "+deptText00+"</font><br>");			

	if (!deptText.equals("")) {

		if (deptText00.equals("")) {//초기화면

		deptText_last = toMulti(deptText)+"<IN>Z_GROUPCODE01";
		//out.println("<br><font color=red>001: deptText=위원회정보있는경우--deptText_last= "+deptText_last+"로 변경</font><br>");	

		} else {

		deptText_last = toMulti(deptText)+"<IN>Z_GROUPCODE01 <AND> "+toMulti(deptText00)+"<IN>Z_GROUPCODE02" ;
		//out.println("<br><font color=red>002: deptText=위원회정보있는경우--deptText_last= "+deptText_last+"로 변경</font><br>");	
...
function GotoPage(form, docStart){	
	...
	{
	document.PageForm.action="./ISearch_Relation.jsp";		
	}
	form.docStart.value = docStart * form.docPage.value + 1;
	form.submit();
	return;
}
function GotoCommittee(form,deptText,deptText00,GROUPNAME,currCommittee,queryText){	
...
<form  method=post name=PageForm action=./history/InsertUserHistoryProc.jsp >
        <tr valign="top"> 
          ...
          <td align="left"><table width="759" border="0" cellspacing="0" cellpadding="0">
              ...
              <tr> 
			<td height="50" colspan="2" align="left" class="text_s">각 위원회별 관련기관의 홈페이지에서 등록되는 각종 정보를 검색 할 수 있는 기능을 제공합니다. 웹로봇이 자동으로 수집한 자료를 최근 자료순으로 보여줍니다.(24시간 마다 자료  Update 됩니다.)</td>
              </tr>
              <tr> 
...
%>
	...
	<%	if (j==3){ %>
                      <td  height="22"><a href="JavaScript:GotoCommittee(document.PageForm,'<%=objHashBSorts.get("GROUPCODE")%>','','<%=objHashBSorts.get("GROUPNAME")%>','<%=objHashBSorts.get("GROUPNAME")%>','<%=query%>')"><%=objHashBSorts.get("GROUPNAME")%></a>(<%=phitCount%>)</td>
                    <tr> 
                    </tr>
	<%	}else{ %>
                      <td  height="22"><a href="JavaScript:GotoCommittee(document.PageForm,'<%=objHashBSorts.get("GROUPCODE")%>','','<%=objHashBSorts.get("GROUPNAME")%>','<%=objHashBSorts.get("GROUPNAME")%>','<%=query%>')"><%=objHashBSorts.get("GROUPNAME")%></a>(<%=phitCount%>)</td>
...
<%
	...
	try {
		...
		String cnt_string="";
		int k=0; 
		//out.println("objArrselectCommitteeUnder.size="+objArrselectCommitteeUnder.size());

		for(int i=0; i < objArrselectCommitteeUnder.size(); i++){  
		Hashtable objHashBSorts = (Hashtable)objArrselectCommitteeUnder.get(i);

		k=i%4;
		if (query.equals("")) {
		cnt_string = objHashBSorts.get("GROUPCODE")+"<IN>Z_GROUPCODE02";
		}else{
		cnt_string = objHashBSorts.get("GROUPCODE")+"<IN>Z_GROUPCODE02<AND>"+toMulti(query);
		}
		//out.println("cnt_string=<br>"+toMulti(cnt_string)+"<br>");
		vs.setQueryText(toAscii(cnt_string));
		result = vs.getResult();	
...
%>
	...
	<%	}else{ %>
                      <td  height="22"><a href="JavaScript:GotoCommittee(document.PageForm,'<%=deptText%>','<%=objHashBSorts.get("GROUPCODE")%>','<%=objHashBSorts.get("GROUPNAME")%>','<%=currCommittee%>','<%=query%>')"><%=objHashBSorts.get("GROUPNAME")%></a>(<%=phitCount%>)</td>
	<%}%>
                    ...
                    <tr> 
                      ...
                      <td width="41%" rowspan="2" align="right" valign="bottom" class="text_s"><table width="20%" border="0" cellspacing="0" cellpadding="0">
                          <tr align="left" valign="bottom"> 
                            <td width="3%"><img src="../image/infosearch/tab_material.gif" width="96" height="24" border=0></td>
                          </tr>
                        </table></td>
                    </tr>
                  </table></td>
...
%>
                    <tr> 
                      <td  height="22"><a href="<%=doc.field("url")%>"  <%if (SRCH_DISPLAY_KIND.trim().equals("0")) { out.println("target='_blank'");}%>><%=doc.rank%>. <font color="E4A644"><strong>[<%=doc.field("GROUPNAME01")%>><%=doc.field("GROUPNAME02")%>><%=doc.field("GROUPNAME03")%>]</strong></font>                     
...
%>                                </td>
              ...
              <tr> 
                <td height="40" colspan="2" align="left"><a href="ISearch_Relation.jsp"><img src="../image/button/bt_goFirst.gif" width="67" height="20" border=0></a></td>
              </tr>
              <tr> 
