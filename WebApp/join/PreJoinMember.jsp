...
<%
	...
	try
	{
		...
		strJuminNoTemp = strJuminNo;
		strJuminNoTemp = objBase64Code.encode(strJuminNoTemp);
	...
	{
		Log.debug.println("Error!!!" + objExcept.getMessage());
...
<%
	if(objHshReqUserInfo.get("STT_CD").equals("000")){
		//out.println("<script> alert('사용자 등록 신청을 하셔야 합니다'); </script>");
		...
		out.println("<script> var retVal = confirm('관리자가 등록 처리를 확인 중입니다. 가입정보를 보시겠습니까?');  if(retVal == false) document.location = '/index.html'; else document.location = '/join/PrintUserInfoCenter.jsp?juminNo="+strJuminNoTemp+"'</script>");
	}else if(objHshReqUserInfo.get("STT_CD").equals("006")){
		out.println("<script> alert('[사용자 등록 확인]을 이용하여 등록을 완료하십시요'); document.location = '/join/Agreement.jsp';</script>");
