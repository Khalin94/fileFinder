...
<%
	...
	String strDataPwd = this.replaceXss(StringUtil.getNVLNULL(multi.getParameter("datapwd")));							//게시물 비밀번호
        //패스워드 필수값 체크
        if (("".equals(strDataPwd) || strDataPwd == null) && strBbrdID.equals("0000000016")) {
		out.println("<script language=javascript>");
		out.println("alert('패스워드 값은 필수값입니다.');");
		...
		if(strCmd.equals("CREATE")){
			...
			if(flagResult == true){
				...
				if(strWriterNm == null || strWriterNm.equals("")) {                     //추가 (2004-06-26)
	             ...
	             } else if ((!strLoginID.equals("ADMIN") || !strLoginID.equals("ADMIN1") || !strLoginID.equals("ADMIN2")) &&
	                            strWriterNm.matches(".*관리자.*") || strWriterNm.matches(".*운영자.*") || strWriterNm.matches(".*시스템.*") || strWriterNm.toUpperCase().matches(".*ADMIN.*")) {
	                    out.println("<script language=javascript>");
	                    out.println("alert('등록자에는 특정 문구를 포함할 수 없습니다.');");
	                    out.println("history.go(-2);");
	                    out.println("</script>");
	                    return;
	            } else {
