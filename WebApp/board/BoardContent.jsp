...
<%
	...
	String strDelGoToPage = URLEncoder.encode("/board/BoardList.jsp", "UTF-8");
...
<%
	...
	if(strEditorExt.equals("N")) {
		strUserNm = nads.lib.reqsubmit.util.StringUtil.getNoTagStr(BoardComm.replaceTag2Text(strUserNm));
	}
			...
			<div id="rightCon">
				...
				<table cellspacing="0" cellpadding="0" class="list03">
					...
					<tr>
						...
						<td height="45" valign="top" colspan="3"><%=strCont%></td>
					</tr>
						...
						<td colspan="5">
							<span class="fonts">
								...
								<%
                                    ...
                                    strNTitle = strNTitle.replaceAll("&amp;lt;","<");
                                    strNTitle = strNTitle.replaceAll("&amp;gt;",">");
									out.println(nads.lib.reqsubmit.util.StringUtil.getNoTagStr(strNTitle));
								%>
			                    </a>
							</span>
						</td>
						...
						<%
							if (objAryTtopList != null) {
								for (int i = 0; i < objAryTtopList.size(); i++) {
                                    ...
                                    strTtopTitle = strTtopTitle.replaceAll("&amp;lt;","<");
                                    strTtopTitle = strTtopTitle.replaceAll("&amp;gt;",">");
									strTtopTitle  = nads.lib.reqsubmit.util.StringUtil.getNoTagStr(strTtopTitle);
									...
									String strUpdTsTemp = (String)objHshTtopList.get("UPD_TS");

									if(!strUpdTsTemp.equals("")){

										strTtopWriteTs = strUpdTsTemp;
...
<%
                        ...
                        <%
								...
								if (strBbrdAnsExt.equals("Y") && strBbrdUserAuth.equals("Y")) {
						%>
	            ...
	            <% if(strBbrdOneExt.equals("Y") && !strDelFlag.equals("A")){ %>
              	<table width="100%" border="0" cellspacing="0" cellpadding="0">
					<form name="frmOneAns" method="post" action="/board/common/BoardOneAnsProc.jsp?cmd=ONE_CREATE" onSubmit="return goToOneSave();">
						...
						<%
							if(strOneExt.equals("Y")){
								...
								try {
									...
									objAryOneAnsList = objBoard.selectOneAnsList(strDataID);
								} catch (AppException objAppEx) {
									// 에러 발생 메세지 페이지로 이동한다.
						...
						%>
						<tr>
							<td align="center" <%=strBgColor%> class="td_box">
								<%=strOneUserNm%><br>
								(<%=strOneWriteTs%>)
							</td>
							<td colspan="2" valign="top" <%=strBgColor%> class="td_box">
								<%=strOneWord%>
								<% if (strLoginID.equals(strOneWriterID) && strOneDelFlag.equals("N")){ %>
								<a href="/board/common/BoardOneAnsProc.jsp?strGoToPage=<%=strGoToPage%>&strCurrentPage=<%=strCurrentPage%>&cmd=ONE_DELETE&bbrdid=<%=strBbrdID%>&dataid=<%=strDataID%>&oneid=<%=strOneID%>&onecnt=<%=intOneCnt%>" onClick="return goToOneDelete();">
									<img src="/image/button/bt_del_board.gif" width="25" height="12" border="0">
								</a>
								<% } %>
							</td>
						</tr>
						...
						<tr>
							<td width="18%" rowspan="2" bgcolor="E9E9E9" class="td_gray2">
								<font color="F5F5F5">....</font>
								<img src="/image/common/icon_nemo_gray.gif" width="3" height="6">
								<span class="soti_forum">한줄 답글</span>
							</td>
							<td width="67%" height="25" valign="bottom" bgcolor="E9E9E9">
								&nbsp;&nbsp;&nbsp;&nbsp;작성자 : <%=strLoginNm%>&nbsp;&nbsp;&nbsp;[현재 <span id="textlimit">0</span>/최대 100byte(한글 50자, 영문 100자)]
							</td>
							<td width="15%" valign="middle" bgcolor="E9E9E9">&nbsp;</td>
						</tr>
						<tr>
							<td width="68%" valign="top" bgcolor="E9E9E9" class="td_box">
								<input name="oneword" type="text" class="textfield" style="WIDTH: 420px" onKeyUp="javascript:textCounter(frmOneAns.oneword, 100);">
							</td>
